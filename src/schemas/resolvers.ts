...
const resolvers = {
  Query: {
    ...
    me: async (_parent: unknown, _args: unknown, context: Context): Promise<Users | null> => {
      if (context.user) {
  ...
  Mutation: {
    ...
    login: async (_parent: unknown, { email, password }: { email: string; password: string }): Promise<{ token: string; user: any }> => {
    ...
    addAsset: async (_parent: unknown, { userId, asset }: AddAssetArgs, context: Context): Promise<Users | null> => {
    ...
    removeUser: async (_parent: unknown, _args: unknown, context: Context): Promise<Users | null> => {
      if (context.user) {
